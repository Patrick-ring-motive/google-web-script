...
(() => {
  ...
  function isValidHeader(key, value) {
    try {
      const headers = {};
      headers[key] = value;
      UrlFetchApp.getRequest('https://is.Valid.Header', {
        headers
      });
      return true;
    } catch (_) {
  ...
  const ResponseConstructor = ($this, body, options = {}) => {
  ...
  const ResponseGetAllHeaders = ($this) => {
  ...
  const Response = class WebResponse extends ContentService.createTextOutput {
    ...
    constructor(body, options = {}) {
      ...
      ResponseConstructor(this, body, options);
     ...
     */
    getAllHeaders() {
      return ResponseGetAllHeaders(this);
    }
  ...
  const ResponseEvent = class WebResponseEvent extends Web.Response {
    ...
    constructor(response) {
      ...
      if (canParseXML(bodyText) || /xml|html/i.test(contentType)) {
        ...
        return new Web.HtmlResponse(bodyText, {
          ...response,
          headers
        });
      }
  ...
  /**
   * Web.HtmlResponse - Wrapper around HtmlService.createHtmlOutput
   *
   * Purpose: Provide a lightweight HtmlService-backed response object that
   * preserves the Web.Response API surface for HTML responses returned from
   * Apps Script `doGet`/`doPost` handlers. This is a minimal stub that can be
   * extended later as needed by the test-suite or user code.
   */
  const HtmlResponse = class WebHtmlResponse extends HtmlService.createHtmlOutput {
    constructor(body, options = {}) {
      if (!body) {
        super();
      } else {
        super(new Web.Blob(body));
      }
      ResponseConstructor(this, body, options);
      // Use the HtmlOutput instance as the backing object by setting its
      // prototype to our HtmlResponse prototype and returning it.
      return Object.setPrototypeOf(this, Web.HtmlResponse.prototype);
    }

    // Provide convenience alias to match Web.Response API
    text() {
    ...
    clone() {
      ...
      // Create new response with cloned data
      const cloned = new Web.HtmlResponse(bodyClone, {
        status: this[$status],
        statusText: this[$statusText],
        headers: headersClone
      });
